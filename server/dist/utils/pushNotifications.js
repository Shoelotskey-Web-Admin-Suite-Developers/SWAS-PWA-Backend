...
const NotifToken_1 = require("../models/NotifToken");
...
async function sendPushNotification(userId, title, body, data) {
    try {
        ...
        const notifToken = await NotifToken_1.NotifToken.findOne({ cust_id: userId }).lean();
...
async function sendBulkNotifications(userIds, title, body, data) {
    try {
        ...
        const notifTokens = await NotifToken_1.NotifToken.find({
            cust_id: { $in: userIds }
        }).lean();
        ...
        const messages = [];
        const userIdToMessageIndex = new Map();
        ...
        for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
            ...
            try {
                const response = await axios_1.default.post(EXPO_PUSH_ENDPOINT, batch, {
                    headers: {
                        'Accept': 'application/json',
                        'Accept-encoding': 'gzip, deflate',
                        'Content-Type': 'application/json',
 ...
 */
function findUserIdByMessageIndex(userIdToMessageIndex, messageIndex) {
...
async function getPushNotificationReceipts(ticketIds) {
    try {
        ...
        const response = await axios_1.default.post(EXPO_RECEIPT_ENDPOINT, { ids: ticketIds }, {
            headers: {
                'Accept': 'application/json',
                'Accept-encoding': 'gzip, deflate',
                'Content-Type': 'application/json',
